...
use acpc_protocol::bridge::{self, BridgeConfig};
use acpc_protocol::protocol::{Command, Event};
use acpc_protocol::{Attachment, BridgeHandle, CommandSender, Settings, SubprocessConfig};
...
fn build_config(agent: &AgentProfile) -> BridgeConfig {
    ...
    if !agent.env.iter().any(|e| e.name == "PATH") {
        ...
        let path =
            format!("{home}/.local/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:{existing}");
